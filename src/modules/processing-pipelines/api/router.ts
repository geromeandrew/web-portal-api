...
import { canonicalizeProcessingPipelineExpectedFileName, matchesProcessingPipelineRequirement } from "../../../processingPipelineCatalog.js";
...
const expectedFileSchema = z.object({ expectedFileName: z.string().min(1).max(512) });
...
const batchRunSchema = z.object({ batchCycle: z.string().regex(/^\d{2}$/, "must be a two-digit bill cycle") });
const allowedExtensions = new Set([".xlsx", ".xls", ".csv", ".txt"]);

function storageError(error: unknown) {
  if (error instanceof Error && error.message === "Requested key is outside the selected processing pipeline folder.") return new AppError(404, "PIPELINE_FILE_NOT_FOUND", "The requested file is not available in the selected pipeline.");
...
export function createProcessingPipelinesRouter({ pool, config, processingPipelineStorage, stepFunctionsRunner, logger }: AppDependencies, singleFile: RequestHandler) {
  ...
  router.get("/:pipelineCode/requirements", async (request, response) => {
    const pipelineCode = pipelineCodeSchema.parse(request.params.pipelineCode); response.json({ requirements: await getProcessingPipelineRequirements(pool, pipelineCode) });
  });
  router.get("/:pipelineCode/execution-details", requireBootstrapAdmin, async (request, response) => {
    const pipelineCode = pipelineCodeSchema.parse(request.params.pipelineCode); const { expectedFileName } = executionDetailsQuerySchema.parse(request.query); const canonicalExpectedFileName = canonicalizeProcessingPipelineExpectedFileName(expectedFileName); const requirement = (await getProcessingPipelineRequirements(pool, pipelineCode)).find((candidate) => candidate.fileName === canonicalExpectedFileName); if (!requirement) throw new AppError(404, "PIPELINE_FILE_NOT_FOUND", "The selected file mapping is not available for this pipeline."); if (!requirement.stepFunction) throw new AppError(404, "PIPELINE_STEP_FUNCTION_NOT_MAPPED", "No Step Functions state machine is mapped to the selected file."); const key = `${pipelineCode}/${requirement.fileName}`;
    let sourceFileExists: boolean;
    try { sourceFileExists = await processingPipelineStorage.fileExists(pipelineCode, key); } catch (error) { throw storageError(error); }
    ...
    ]; response.json({ pipelineCode, expectedFileName: requirement.fileName, sourceFile: { key, s3Uri: `s3://${config.S3_BUCKET}/${key}`, exists: sourceFileExists }, execution: { input: requirement.stepFunction.executionInput, stateMachine }, canExecute: blockingReasons.length === 0, blockingReasons }); } catch (error) { throw stepFunctionError(error); }
  });
  router.get("/:pipelineCode/batch-execution-details", requireBootstrapAdmin, async (request, response) => {
    const pipelineCode = pipelineCodeSchema.parse(request.params.pipelineCode); const { batchCycle } = batchRunSchema.parse(request.query); const mapping = await getProcessingPipelineBatchStepFunctionMapping(pool, pipelineCode, batchCycle); if (!mapping) throw new AppError(404, "PIPELINE_BATCH_STEP_FUNCTION_NOT_MAPPED", "No batch Step Functions state machine is mapped to the selected bill cycle.");
    let sourceFiles: { expectedFileName: string; displayOrder: number; key: string; s3Uri: string; exists: boolean }[];
    try { sourceFiles = await Promise.all(mapping.files.map(async (file) => { const key = `${pipelineCode}/${file.expectedFileName}`; return { ...file, key, s3Uri: `s3://${config.S3_BUCKET}/${key}`, exists: await processingPipelineStorage.fileExists(pipelineCode, key) }; })); } catch (error) { throw storageError(error); }
    try { const stateMachine = await stepFunctionsRunner.describeStateMachine(mapping.stateMachineName); const blockingReasons = [
      ...
      ...(stateMachine.status === "ACTIVE" ? [] : ["STATE_MACHINE_INACTIVE"]),
      ...(stateMachine.type === "STANDARD" ? [] : ["STATE_MACHINE_NOT_STANDARD"]),
    ]; response.json({ pipelineCode, batchCycle: mapping.batchCycle, sourceFiles, execution: { input: mapping.executionInput, stateMachine }, canExecute: blockingReasons.length === 0, blockingReasons }); } catch (error) { throw stepFunctionError(error); }
  });
  router.get("/:pipelineCode/files", async (request, response) => {
    ...
    try { const requirements = await getProcessingPipelineRequirements(pool, pipelineCode); const files = requirements.length ? await processingPipelineStorage.listExpectedFiles(pipelineCode, requirements) : []; response.json(requirements.length ? { configured: true, files: attachConfiguration(files, requirements, config.S3_BUCKET) } : { configured: false, files: [] }); } catch (error) { throw storageError(error); }
  });
  router.get("/:pipelineCode/files/content", async (request, response) => {
    const pipelineCode = pipelineCodeSchema.parse(request.params.pipelineCode); const { key } = contentQuerySchema.parse(request.query);
    try { const requirements = await getProcessingPipelineRequirements(pool, pipelineCode); const prefix = `${pipelineCode}/`; const fileName = key.startsWith(prefix) ? key.slice(prefix.length) : ""; if (!requirements.some((requirement) => matchesProcessingPipelineRequirement(requirement, fileName))) throw new AppError(404, "PIPELINE_FILE_NOT_FOUND", "The requested file is not configured for this pipeline."); const file = await processingPipelineStorage.getFile(pipelineCode, key); if (!file.Body || typeof (file.Body as Readable).pipe !== "function") throw new AppError(502, "PIPELINE_STORAGE_INVALID", "The pipeline storage service returned an invalid file response."); const filename = key.split("/").at(-1) ?? "file"; response.status(200).set({ "Cache-Control": "private, no-store", "Content-Disposition": `inline; filename*=UTF-8''${encodeURIComponent(filename)}`, "Content-Type": file.ContentType ?? "application/octet-stream" }); if (file.ContentLength !== undefined) response.set("Content-Length", String(file.ContentLength)); await pipeline(file.Body as Readable, response); } catch (error) { throw storageError(error); }
  });
  router.post("/:pipelineCode/files", singleFile, async (request, response) => {
    if (!request.file) throw new AppError(400, "FILE_REQUIRED", "Choose a file to upload."); const pipelineCode = pipelineCodeSchema.parse(request.params.pipelineCode); const { expectedFileName, replace } = uploadSchema.parse(request.body); const extension = request.file.originalname.slice(request.file.originalname.lastIndexOf(".")).toLowerCase();
    if (!allowedExtensions.has(extension)) throw new AppError(415, "PIPELINE_FILE_TYPE", "Pipeline uploads must be .xlsx, .xls, .csv, or .txt files."); const canonicalExpectedFileName = canonicalizeProcessingPipelineExpectedFileName(expectedFileName); const requirement = (await getProcessingPipelineRequirements(pool, pipelineCode)).find((candidate) => candidate.fileName === canonicalExpectedFileName); if (!requirement) throw new AppError(404, "PIPELINE_FILE_NOT_FOUND", "The selected file mapping is not available for this pipeline."); const localName = request.file.originalname.split(/[\\/]/).at(-1) ?? "upload.bin"; const normalizedLocalName = canonicalizeProcessingPipelineExpectedFileName(localName); const savedName = requirement.match === "exact" ? requirement.fileName : normalizedLocalName; if (requirement.match === "glob" && !matchesProcessingPipelineRequirement(requirement, savedName)) throw new AppError(400, "PIPELINE_FILENAME_MISMATCH", `The selected filename must match ${requirement.fileName}.`);
    const key = `${pipelineCode}/${savedName}`;
    try { const exists = await processingPipelineStorage.fileExists(pipelineCode, key); if (exists && replace !== "true") throw new AppError(409, "PIPELINE_FILE_EXISTS", "A file already exists at this location. Confirm replacement and try again."); const stored = await processingPipelineStorage.uploadFile(pipelineCode, savedName, new Uint8Array(request.file.buffer), request.file.mimetype || "application/octet-stream"); const workspaceId = await ensureWorkspace(pool, request.auth!.userId); await audit(pool, workspaceId, "processing-pipeline.file-uploaded", { pipelineCode, expectedFileName: requirement.fileName, originalName: localName, objectKey: stored.key, replaced: exists }); response.status(201).json({ file: stored, renamed: requirement.match === "exact" && localName !== savedName, replaced: exists }); } catch (error) { throw storageError(error); }
  });
  router.post("/:pipelineCode/runs", requireBootstrapAdmin, async (request, response) => {
    const pipelineCode = pipelineCodeSchema.parse(request.params.pipelineCode); const { expectedFileName } = expectedFileSchema.parse(request.body); const canonicalExpectedFileName = canonicalizeProcessingPipelineExpectedFileName(expectedFileName); const requirement = (await getProcessingPipelineRequirements(pool, pipelineCode)).find((candidate) => candidate.fileName === canonicalExpectedFileName); if (!requirement) throw new AppError(404, "PIPELINE_FILE_NOT_FOUND", "The selected file mapping is not available for this pipeline."); if (!requirement.stepFunction) throw new AppError(404, "PIPELINE_STEP_FUNCTION_NOT_MAPPED", "No Step Functions state machine is mapped to the selected file."); if (requirement.match !== "exact") throw new AppError(400, "PIPELINE_FILE_PATTERN_UNSUPPORTED", "A specific file must be selected before starting its processing run."); const key = `${pipelineCode}/${requirement.fileName}`;
    try { if (!(await processingPipelineStorage.fileExists(pipelineCode, key))) throw new AppError(404, "PIPELINE_FILE_NOT_FOUND", "Upload the selected file before starting its processing run."); } catch (error) { throw storageError(error); }
    const runId = randomUUID(); const workspaceId = await ensureWorkspace(pool, request.auth!.userId);
    try { const stateMachine = await stepFunctionsRunner.describeStateMachine(requirement.stepFunction.stateMachineName); if (stateMachine.status !== "ACTIVE" || stateMachine.type !== "STANDARD") throw new AppError(409, "PIPELINE_STEP_FUNCTION_NOT_RUNNABLE", "The mapped Step Functions state machine must be active and Standard before it can be run."); const started = await stepFunctionsRunner.startExecution(requirement.stepFunction.stateMachineName, requirement.stepFunction.executionInput); const sourceFiles = [{ expectedFileName: requirement.fileName, key, s3Uri: `s3://${config.S3_BUCKET}/${key}` }]; await createStepFunctionExecution(pool, { id: runId, fileMappingId: requirement.stepFunction.mappingId, executionArn: started.executionArn, executionInput: requirement.stepFunction.executionInput, sourceFiles, workspaceId, startedByUserId: request.auth!.userId, status: "RUNNING", startedAt: started.startedAt }); try { await audit(pool, workspaceId, "processing-pipeline.step-function-started", { pipelineCode, expectedFileName: requirement.fileName, objectKey: key, stateMachineName: requirement.stepFunction.stateMachineName, executionInput: requirement.stepFunction.executionInput, runId, executionArn: started.executionArn }); } catch (error) { logger.error("processing_pipeline.step_function_audit_failed", { error: String(error) }); } response.status(201).json({ runId, targetMode: "adhoc", stateMachineName: requirement.stepFunction.stateMachineName, executionInput: requirement.stepFunction.executionInput, startedAt: started.startedAt }); } catch (error) { throw stepFunctionError(error); }
  });
  router.post("/:pipelineCode/batch-runs", requireBootstrapAdmin, async (request, response) => {
    const pipelineCode = pipelineCodeSchema.parse(request.params.pipelineCode); const { batchCycle } = batchRunSchema.parse(request.body); const mapping = await getProcessingPipelineBatchStepFunctionMapping(pool, pipelineCode, batchCycle); if (!mapping) throw new AppError(404, "PIPELINE_BATCH_STEP_FUNCTION_NOT_MAPPED", "No batch Step Functions state machine is mapped to the selected bill cycle.");
    let sourceFiles: { expectedFileName: string; displayOrder: number; key: string; s3Uri: string }[];
    try { const resolved = await Promise.all(mapping.files.map(async (file) => { const key = `${pipelineCode}/${file.expectedFileName}`; return { ...file, key, s3Uri: `s3://${config.S3_BUCKET}/${key}`, exists: await processingPipelineStorage.fileExists(pipelineCode, key) }; })); if (resolved.some((file) => !file.exists)) throw new AppError(409, "PIPELINE_BATCH_SOURCE_FILES_MISSING", "Upload all five mapped batch source files before starting the batch execution."); sourceFiles = resolved.map(({ exists: _exists, ...file }) => file); } catch (error) { throw storageError(error); }
    const runId = randomUUID(); const workspaceId = await ensureWorkspace(pool, request.auth!.userId);
    try { const stateMachine = await stepFunctionsRunner.describeStateMachine(mapping.stateMachineName); if (stateMachine.status !== "ACTIVE" || stateMachine.type !== "STANDARD") throw new AppError(409, "PIPELINE_STEP_FUNCTION_NOT_RUNNABLE", "The mapped Step Functions state machine must be active and Standard before it can be run."); const started = await stepFunctionsRunner.startExecution(mapping.stateMachineName, mapping.executionInput); await createStepFunctionExecution(pool, { id: runId, batchMappingId: mapping.mappingId, executionArn: started.executionArn, executionInput: mapping.executionInput, sourceFiles, workspaceId, startedByUserId: request.auth!.userId, status: "RUNNING", startedAt: started.startedAt }); try { await audit(pool, workspaceId, "processing-pipeline.batch-step-function-started", { pipelineCode, batchCycle, sourceFiles, stateMachineName: mapping.stateMachineName, executionInput: mapping.executionInput, runId, executionArn: started.executionArn }); } catch (error) { logger.error("processing_pipeline.batch_step_function_audit_failed", { error: String(error) }); } response.status(201).json({ runId, targetMode: "batch", stateMachineName: mapping.stateMachineName, executionInput: mapping.executionInput, sourceFiles, startedAt: started.startedAt }); } catch (error) { throw stepFunctionError(error); }
  });
