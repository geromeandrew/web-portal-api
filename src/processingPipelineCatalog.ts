...
add("Memo Standard Template", "memo_sst", memoFiles.flatMap((name) => ["01", "02", "03"].map((suffix) => [`${name}_${suffix}.XLSX`, "Memo_Format04_38Columns.dtsx", null] as Definition)));
...
add("ICCBS - Bayan", "iccbs_bayn", [
  ...["01", "02", "10", "12"].flatMap((month) => ["03", "05", "06", "08", "12", "15", "16", "21", "26", "31"].map((cycle) => [`bl_act_trans_ext_BT_${month}${cycle}.txt`, "ICCBS_bl_act_trans_ext_BT_mmdd.dtsx", null] as Definition)),
...
add("Prepaid Reclass", "prepaid_reclass", [
  ...[["308. Billed Adjustments Monthly Summary Report_G_BC01.XLSX", "Prepaid_308_Billed_Adj.dtsx"], ["318. Billed Charges Summary Report_G_BC01.XLSX", "Prepaid_318_Billed_Charges.dtsx"]].flatMap(([fileName, packageName]) => bayanBillCycleSuffixes.map((suffix) => [fileName.replace("01.XLSX", `${suffix}.XLSX`), packageName] as Definition)),
  ["CALLCARD_SG.XLSX", "Prepaid_CallCard_SG.dtsx"], ["CALLCARD_EG.XLSX", "Prepaid_CallCard_EG.dtsx"], ["LOAD API_EG.XLSX", "Prepaid_Load_API_EG.dtsx"], ["Load API_SG.XLSX", "Prepaid_Load_API_SG.dtsx"], ["LOAD UP_EG.XLSX", "Prepaid_Load_UP_EG.dtsx"], ["Load UP_SG.XLSX", "Prepaid_Load_UP_SG.dtsx"], ["DL_Monthly_Recharge_summary_report.csv", "Prepaid_DL_Monthly_Recharge.dtsx"], ["KE24.XLSX", "Prepaid_KE24.dtsx"],
]);
...
export function canonicalizeProcessingPipelineExpectedFileName(fileName: string) {
