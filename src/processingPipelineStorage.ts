...
export const processingPipelineStages = ["inbound", "outbound", "processed", "error"] as const;
export type ProcessingPipelineStage = (typeof processingPipelineStages)[number];
...
export class ProcessingPipelineStorage {
  ...
  private prefix(pipelineCode: string, stage: ProcessingPipelineStage) { return `${pipelineCode}/${stage}/`; }

  async listFiles(pipelineCode: string, stage: ProcessingPipelineStage, cursor?: string) {
    const prefix = this.prefix(pipelineCode, stage);
  ...
  async listAllFiles(pipelineCode: string, stage: ProcessingPipelineStage) {
    const files: ProcessingPipelineFile[] = []; let cursor: string | undefined;
    do { const page = await this.listFiles(pipelineCode, stage, cursor); files.push(...page.files); cursor = page.nextCursor; } while (cursor);
    return files;
  }

  async listExpectedFiles(pipelineCode: string, stage: ProcessingPipelineStage, requirements: readonly ProcessingPipelineFileRequirement[]): Promise<ProcessingPipelineExpectedFile[]> {
    const files = await this.listAllFiles(pipelineCode, stage);
    return requirements.flatMap<ProcessingPipelineExpectedFile>((requirement, requirementIndex) => {
      ...
      if (!matches.length) return [{ id: `${requirementIndex}:missing`, stage, expectedFileName: requirement.fileName, matchedFileName: null, legacyPackageName: requirement.legacyPackageName, jobName: requirement.jobName, availability: "missing" as const, key: null, size: null, lastModified: null }];
      return matches.map((file, matchIndex) => ({ id: `${requirementIndex}:${matchIndex}:${file.key}`, stage, expectedFileName: requirement.fileName, matchedFileName: file.name, legacyPackageName: requirement.legacyPackageName, jobName: requirement.jobName, availability: "present" as const, key: file.key, size: file.size, lastModified: file.lastModified }));
    });
  }

  async getFile(pipelineCode: string, stage: ProcessingPipelineStage, key: string) {
    const prefix = this.prefix(pipelineCode, stage);
  ...
  async fileExists(pipelineCode: string, stage: ProcessingPipelineStage, key: string) {
    const prefix = this.prefix(pipelineCode, stage);
    if (!key.startsWith(prefix) || key === prefix || key.endsWith("/")) throw new Error("Requested key is outside the selected processing pipeline folder.");
  ...
  async uploadFile(pipelineCode: string, stage: ProcessingPipelineStage, name: string, body: Uint8Array, contentType: string) {
    const key = `${this.prefix(pipelineCode, stage)}${name}`;
