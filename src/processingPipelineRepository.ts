...
type RequirementRow = {
  ...
  step_function_mapping_id: number | null; state_machine_name: string | null; batch_cycle: string | null; execution_input: unknown;
};
...
export type PipelineRequirementDto = ProcessingPipelineFileRequirement & {
  ...
  stepFunction: { mappingId: number; stateMachineName: string; batchCycle: string | null; executionInput: Record<string, unknown> } | null;
};
...
function requirementDto(pipelineCode: string, row: RequirementRow): PipelineRequirementDto {
  return {
    ...
    stepFunction: row.step_function_mapping_id && row.state_machine_name ? { mappingId: row.step_function_mapping_id, stateMachineName: row.state_machine_name, batchCycle: row.batch_cycle, executionInput: jsonObject(row.execution_input) } : null,
  };
}
...
export type StepFunctionExecution = {
  id: string; executionArn: string; targetMode: "adhoc" | "batch"; stateMachineName: string; executionInput: Record<string, unknown>; sourceFiles: Record<string, unknown>[];
  status: string; errorCode: string | null; errorMessage: string | null; output: string | null; outputIncluded: boolean | null; startedAt: string; completedAt: string | null;
};

export async function createStepFunctionExecution(pool: Pool, execution: {
  id: string; fileMappingId?: number; batchMappingId?: number; executionArn: string; executionInput: Record<string, unknown>; sourceFiles: Record<string, unknown>[]; workspaceId: string; startedByUserId: string; status: string; startedAt: string;
}) {
  await pool.query(
    ...
    [execution.id, execution.fileMappingId ?? null, execution.batchMappingId ?? null, execution.executionArn, typeof execution.sourceFiles[0]?.key === "string" ? execution.sourceFiles[0].key : null, JSON.stringify(execution.executionInput), JSON.stringify(execution.sourceFiles), execution.workspaceId, execution.startedByUserId, execution.status, execution.startedAt],
  );
}

export async function getStepFunctionExecution(pool: Pool, id: string, pipelineCode: string): Promise<StepFunctionExecution | null> {
  const result = await pool.query<{ id: string; execution_arn: string; target_mode: "adhoc" | "batch"; state_machine_name: string; execution_input: unknown; source_files: unknown; status: string; error_code: string | null; error_message: string | null; execution_output: string | null; output_included: boolean | null; started_at: Date; completed_at: Date | null }>(
  ...
  return row ? { id: row.id, executionArn: row.execution_arn, targetMode: row.target_mode, stateMachineName: row.state_machine_name, executionInput: jsonObject(row.execution_input), sourceFiles: jsonArray<Record<string, unknown>>(row.source_files), status: row.status, errorCode: row.error_code, errorMessage: row.error_message, output: row.execution_output, outputIncluded: row.output_included, startedAt: row.started_at.toISOString(), completedAt: row.completed_at?.toISOString() ?? null } : null;
}

export async function updateStepFunctionExecution(pool: Pool, id: string, observed: { status: string; errorCode: string | null; errorMessage: string | null; output: string | null; outputIncluded: boolean | null; completedAt: string | null }) {
  await pool.query(
    ...
    [id, observed.status, observed.errorCode, observed.errorMessage, observed.output, observed.outputIncluded, observed.completedAt],
  );
}
