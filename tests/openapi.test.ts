...
describe("OpenAPI documentation", () => {
  it("is a valid OpenAPI document covering the currently exposed Swagger routes", async () => {
    ...
    expect(
      openApiDocument.paths["/api/processing-pipelines/{pipelineCode}/files"]
        ?.get?.parameters,
    ).toHaveLength(1);
    expect(
    ...
    for (const [pathName, path] of Object.entries(openApiDocument.paths)) {
      if (!path || "$ref" in path) continue;
      const names = [...pathName.matchAll(/\{([^}]+)\}/g)].map(
        (match) => match[1],
      );
      for (const operation of [
        path.get,
  ...
  it("can restore non-essential endpoints in Swagger through configuration", () => {
    ...
    expect(
      fullDocument.paths["/api/workflows/prepaid/report.csv"]?.get?.responses[
        "200"
      ].content?.["text/csv"],
    ).toBeDefined();
  });
  ...
  it("serves the OpenAPI JSON and Swagger UI", async () => {
    ...
    try {
      const [rootResponse, specResponse, docsResponse, stylesheetResponse] =
        await Promise.all([
